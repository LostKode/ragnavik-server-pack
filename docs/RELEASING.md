...
For releases that span Shared, Server, Client, or UI packages, follow [the coordinated release train](RELEASE_TRAINS.md).

1. Update `manifest.json`, `CHANGELOG.md`, and the version description in `README.md`.
2. Update `manifests/shared-manifest.json` and `manifests/client-manifest.json` to the exact coordinated releases. Client and Server must pin the same Shared version and must not depend on each other.
3. Audit every effective Client, Shared, and Server component in `manifests/anticheat-policy.json`. Every client-only plugin GUID belongs in the generated extra whitelist. Every genuinely server-only plugin GUID belongs in the generated server-only list.
