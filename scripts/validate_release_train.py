...
def validate(train_path: Path, check_blog: bool) -> dict:
    ...
    expected_top = {
        "schema_version", "release_id", "blog_url", "packages",
        "deployment_required", "deployment_window",
    }
    ...
    if not isinstance(window, dict) or set(window) != {"timezone", "start", "duration_minutes"}:
        fail("deployment_window must contain timezone, start, and duration_minutes")
    if window["timezone"] != "America/Chicago" or window["start"] != "01:00":
        fail("deployment window must start at 01:00 America/Chicago")
    if not isinstance(window["duration_minutes"], int) or not 1 <= window["duration_minutes"] <= 240:
        fail("deployment_window.duration_minutes must be between 1 and 240")
...
def main() -> None:
    ...
    if args.github_output:
        with args.github_output.open("a", encoding="utf-8") as output:
            ...
            output.write(f"deployment_window={json.dumps(train['deployment_window'], separators=(',', ':'))}\n")
